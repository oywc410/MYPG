...
image_size = load.image_size # 图片大小
num_labels = load.num_labels # 图片标签
num_channels = load.num_channels # 通道数
...
class Network():
    def __init__(self, num_hidden, batch_size):
        '''
        :param num_hidden: 隐藏层的节点数量
        :param batch_size: 因为我们要节省内存，所以需要分批处理数据，每一批的数据量
        '''
    ...
    def define_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.tf_train_labels = tf.placeholder(
                tf.float32, shape=(self.batch_size, num_labels) # 0 ～ 9
            )
            self.tf_test_samples = tf.placeholder(
                tf.float32, shape=(self.test_batch_size, image_size, image_size, num_channels)
            )

            # fully connected layer 1, fully connected 隐藏
            fc1_weights = tf.Variable(
                # 随机从高斯分布中取值
                # image_size * image_size 输入个数 num_hidden 输出个数
            ...
            # fully connected layer 2 --> output layer 输出
            fc2_weights = tf.Variable(
                #num_hidden 输入个数 num_labels 输出个数
            ...
            def model(data):
                ...
                print("model:", data.get_shape(), shape)
                reshape = tf.reshape(data, [shape[0], shape[1] * shape[2] * shape[3]]) #转换数据为二维格式  (数据, [格式]) (self.batch_size, [image_size x image_size x num_channels])
                print("model:", reshape.get_shape(), fc1_weights.get_shape(), fc1_biases.get_shape())
    ...
    def accuracy(self, predictions, labels, need_confusion_matrix=False):
        ...
        '''
        _predictions = np.argmax(predictions, 1) #取出每列最大数 [0, 0, 0, 1, 0, 0, 0] => 3
