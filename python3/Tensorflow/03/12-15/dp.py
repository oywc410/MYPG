...
class Network():
	def __init__(self, num_hidden, batch_size, patch_size, conv_depth, pooling_scale):
		'''
		@num_hidden: 隐藏层的节点数量
		...
		with self.graph.as_default():
			...
			with tf.name_scope('fc1'):
				...
				fc1_weights = tf.Variable(
				    tf.truncated_normal( # truncated_normal 随机正态分布
			...
			def model(data):
				with tf.name_scope('conv1_model'):
					#卷积步骤 data 输入数据 filter权重 strides滑窗移动步数 padding抽取模式 SAME or VALID (SAME为 四周补0)
