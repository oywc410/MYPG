...
func cmd_thumbnail(file string, width uint, height uint, to string) {
	...
	jpeg.Encode(file_out, canvas, &jpeg.Options{100})
}
...
func cmd_watermark(file string, to string) {
	...
	// 打开水印图并解码
	file_watermark, _ := os.Open("logo.png")
	...
	// 贴水印图
	draw.Draw(canvas, watermark.Bounds().Add(image.Pt(90, 150)), watermark, image.ZP, draw.Over)

	//生成新图片
	create_image, _ := os.Create(to)
	jpeg.Encode(create_image, canvas, &jpeg.Options{100})
