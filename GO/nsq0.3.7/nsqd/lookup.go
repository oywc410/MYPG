...
func (n *NSQD) lookupLoop() {
	...
	for {
		...
		case val := <-n.notifyChan://生成命令(话题/通道相关) 向nsqlookupd广播命令
			var cmd *nsq.Command
			var branch string
			...
			case *Channel://通道类型命令
				...
				branch = "channel"
				channel := val.(*Channel)
				if channel.Exiting() == true {//通道关闭命令
					//生成删除话题中通道命令
		...
		case lookupPeer := <-syncTopicChan://向单一的lookup广播所有topic创建命令
		...
		case <-n.optsNotificationChan: //添加lookupd节点
